...
const ICON_PATHS = {
  ...
  // currentColor-adaptive (the badge, not the glyph, carries WhatsApp's
  // brand colors -- see .contact-widget-item-icon.is-whatsapp in
  // styles.css), but a bare outer silhouette with no internal detail (the
  // previous version, right after dropping the hardcoded circle+phone
  // version for doubling up inside this app's own icon badges) read as an
  // odd, undefined blob rather than recognizably WhatsApp. A plain circle
  // (not the tailed bubble -- simpler, no stray asymmetric point) with the
  // phone-handset shape punched out via fill-rule=evenodd restores that
  // detail: wherever this sits on a light background (this app's own
  // pastel icon badges, a white dropdown panel, ...) the cutout reads as a
  // clean two-tone glyph again, without needing a second hardcoded color.
  whatsapp:
    '<path fill-rule="evenodd" clip-rule="evenodd" fill="currentColor" stroke="none" d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2Z M8.5 8.3c.2-.5.5-.5.7-.5h.5c.2 0 .4 0 .6.4.2.5.6 1.6.7 1.7.1.1.1.3 0 .5-.1.2-.2.3-.4.5-.2.2-.4.3-.2.6.2.3.8 1.3 1.7 2.1 1.2 1 2.1 1.3 2.5 1.5.3.1.5.1.7-.1.2-.2.8-.9 1-1.2.2-.3.4-.2.6-.1.2.1 1.5.7 1.8.9.3.1.4.2.5.3.1.2.1 1-.3 1.9-.4.9-2.1 1.7-2.9 1.8-.7.1-1.6.2-4.6-1-3.7-1.5-6-5.3-6.2-5.6-.2-.3-1.5-2-1.5-3.8 0-1.8.9-2.6 1.3-3z"/>',
